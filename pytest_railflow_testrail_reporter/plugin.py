...
def restructure(data):
    ...
    for i in data:
        if isinstance(i, OrderedDict):
            restructured_dict = OrderedDict([('railflow_test_attributes', OrderedDict(temp_list))])
                ...
                for i in mark.kwargs:
                    if item.cls:
                        if i in self.fun_list:
                            ...
                            warnings.warn("%s is not a valid class attribute" % i, UserWarning)
                    else:
                        if i in self.fun_list:
                            self.results.append((i,mark.kwargs[i]))
                        else:
                            warnings.warn("%s is not a valid test attribute" % i, UserWarning)
